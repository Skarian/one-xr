...
import io.onepro.xr.XrControlEvent
import io.onepro.xr.XrControlProtocolException
import io.onepro.xr.XrDimmerLevel
import io.onepro.xr.XrDeviceConfigErrorCode
import io.onepro.xr.XrDeviceConfigException
import io.onepro.xr.XrDisplayInputMode
import io.onepro.xr.XrSceneMode
import io.onepro.xrprobe.databinding.ActivityControlsDemoBinding
...
class ControlsDemoActivity : AppCompatActivity() {
    ...
    private fun attachControlEventCollector(xrClient: OneProXrClient) {
        controlEventsJob?.cancel()
    ...
    private fun runControlCommand(
        commandName: String,
        action: suspend (OneProXrClient) -> Unit
    ) {
        val xrClient = client
        if (xrClient == null) {
